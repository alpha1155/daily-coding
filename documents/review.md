...
**Array（数组）并不属于 Java 集合框架（Collection Framework）** 的一部分。

它是 Java 语言内置的基础数据结构，与集合类有本质区别。

### 数组与集合类的核心区别 

虽然数组不属于集合框架，但集合类底层常依赖数组实现：

- `ArrayList` 底层是动态扩容的数组；
- `HashMap` 底层是数组 + 链表 / 红黑树（JDK 1.8）；
- 集合框架提供了 `Arrays.asList()` 方法，可将数组转为 `List`（但返回的是固定大小的列表，属于集合框架）。

### 总结

1. **数组（Array）** 是 Java 内置的基础数据结构，不属于集合框架，长度固定、功能简单；
2. **集合类** 是对数组的封装和扩展，属于 `java.util` 框架，支持动态扩容、泛型和丰富的操作方法；
3. 数组与集合类并非包含关系，但集合类常以数组为底层实现，是对数组功能的补充和增强。
